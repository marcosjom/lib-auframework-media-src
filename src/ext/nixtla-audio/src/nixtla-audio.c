...
struct STNixSharedPtr_* NixSharedPtr_alloc(STNixContextItf* ctx, void* opq){
...
STNixApiEngineRef  NixApiEngineItf_nop_alloc(STNixContextItf* ctx) { return (STNixApiEngineRef)STNixApiEngineRef_Zero; }
...
STNixApiBufferRef  NixApiBufferItf_nop_alloc(STNixContextItf* ctx, const STNixAudioDesc* audioDesc, const NixUI8* audioDataPCM, const NixUI32 audioDataPCMBytes) { return (STNixApiBufferRef)STNixApiBufferRef_Zero; }
...
NixBOOL         NixApiRecorderItf_nop_setCallback(STNixApiRecorderRef ref, NixApiRecorderCallback callback, void* callbackData) { return NIX_FALSE; }
...
NixBOOL NixPCMBuffer_setData(STNixPCMBuffer* obj, const STNixAudioDesc* audioDesc, const NixUI8* audioDataPCM, const NixUI32 audioDataPCMBytes){
    ...
    if(audioDesc != NULL && audioDesc->blockAlign > 0){
        ...
        if(reqBytes <= 0){
            ...
            if(obj->ptr == NULL){
                obj->ptr = (NixUI8*)(*obj->ctx.mem.malloc)(reqBytes, "NixPCMBuffer_setData::ptr");
                if(obj->ptr != NULL){
                    obj->sz = reqBytes;
                }
            }
...
STNixApiBufferRef nixPCMBuffer_alloc(STNixContextItf* ctx, const STNixAudioDesc* audioDesc, const NixUI8* audioDataPCM, const NixUI32 audioDataPCMBytes);
...
STNixApiBufferRef nixPCMBuffer_alloc(STNixContextItf* ctx, const STNixAudioDesc* audioDesc, const NixUI8* audioDataPCM, const NixUI32 audioDataPCMBytes){
    ...
    if(audioDesc != NULL && audioDesc->blockAlign > 0){
        STNixPCMBuffer* obj = (STNixPCMBuffer*)(*ctx->mem.malloc)(sizeof(STNixPCMBuffer), "STNixPCMBuffer");
        ...
        if(obj != NULL){
            ...
            if(!NixPCMBuffer_setData(obj, audioDesc, audioDataPCM, audioDataPCMBytes)){
                ...
                itf = (STNixApiBufferItf*)(*ctx->mem.malloc)(sizeof(STNixApiBufferItf), "STNixApiBufferItf");
                if(itf == NULL){
...
typedef struct STNix_EngineObjetcs_ {
	...
	//Audio capture
    STNixApiRecorderRef    apiRecorder;
	NixUI8				captureInProgess;
	NixUI32				captureSamplesPerBuffer;
	STNixAudioDesc		captureFormat;
	//Capture buffers
...
NixBOOL __nixBufferSetData(STNix_EngineObjetcs* eng, STNix_bufferAL* buffer, const STNixAudioDesc* audioDesc, const NixUI8* audioDataPCM, const NixUI32 audioDataPCMBytes);
...
NixUI16 __nixBufferCreate(STNix_EngineObjetcs* eng){
	...
	if(eng->buffersArrUse < NIX_BUFFERS_MAX){
		if(eng->buffersArrSize >= eng->buffersArrUse){
            ...
            buffersArr = (STNix_bufferAL*)(*eng->ctx.mem.malloc)(sizeof(STNix_bufferAL) * eng->buffersArrSize, "buffersArr");
			if(eng->sourcesArr != NULL){
				if(eng->sourcesArrUse != 0) memcpy(buffersArr, eng->buffersArr, sizeof(STNix_bufferAL) * eng->buffersArrUse);
                (*eng->ctx.mem.free)(eng->buffersArr);
			}
...
void __nixBufferDestroy(STNix_bufferAL* buffer, const NixUI16 bufferIndex){
    ...
    if(buffer->api.free != NULL){
        (*buffer->api.free)(buffer->apiBuff);
        buffer->apiBuff = (STNixApiBufferRef)STNixApiBufferRef_Zero;
    }
	if(buffer->bufferDesc.dataPointer != NULL){
        (*buffer->ctx->mem.free)(buffer->bufferDesc.dataPointer);
...
NixBOOL __nixBufferSetData(STNix_EngineObjetcs* eng, STNix_bufferAL* buffer, const STNixAudioDesc* audioDesc, const NixUI8* audioDataPCM, const NixUI32 audioDataPCMBytes){
    NixBOOL r = NIX_FALSE;
    if(buffer->apiBuff.ptr == NULL){
        if(eng->api.buffer.alloc != NULL){
            buffer->apiBuff = (*eng->api.buffer.alloc)(&eng->ctx, audioDesc, audioDataPCM, audioDataPCMBytes);
            if(buffer->apiBuff.ptr == NULL){
                NIX_PRINTF_ERROR("buffer->apiBuff.ptr is NULL.\n");
            } else {
    ...
    if(r){
        ...
        buffer->bufferDesc.dataPointer = (NixUI8*)(*buffer->ctx->mem.malloc)(sizeof(NixUI8) * audioDataPCMBytes, "bufferDesc.dataPointer");
        if(audioDataPCM != NULL){
...
NixUI16 __nixSourceAdd(STNix_EngineObjetcs* eng, STNix_source* source){
	...
	if(eng->sourcesArrUse < NIX_SOURCES_MAX){
		if(eng->sourcesArrSize >= eng->sourcesArrUse){
            ...
            sourcesArr = (STNix_source*)(*eng->ctx.mem.malloc)(sizeof(STNix_source) * eng->sourcesArrSize, "sourcesArr");
			if(eng->sourcesArr != NULL){
				if(eng->sourcesArrUse != 0) memcpy(sourcesArr, eng->sourcesArr, sizeof(STNix_source) * eng->sourcesArrUse);
                (*eng->ctx.mem.free)(eng->sourcesArr);
			}
...
void __nixSourceFinalize(STNix_Engine* engAbs, STNix_source* source, const NixUI16 sourceIndex, const NixBOOL forReuse){
	...
	if(source->queueBuffIndexes != NULL){
        ...
        (*eng->ctx.mem.free)(source->queueBuffIndexes);
	...
	if(!forReuse){
        if(source->apiSource.ptr != NULL){
            if(source->api.free != NULL){
                (*source->api.free)(source->apiSource);
            }
            source->apiSource = (STNixApiSourceRef)STNixApiSourceRef_Zero;
        }
...
void __nixSrcQueueAddBuffer(STNix_source* src, STNix_bufferAL* buffer, const NixUI16 buffIndex){
	if(src->queueBuffIndexesSize <= src->queueBuffIndexesUse){
        ...
        buffArr = (NixUI16*)(*src->ctx->mem.malloc)(sizeof(NixUI16) * src->queueBuffIndexesSize, "queueBuffIndexes");
		if(src->queueBuffIndexes != NULL){
            ...
            (*src->ctx->mem.free)(src->queueBuffIndexes);
		}
...
NixBOOL nixInitBase_(STNix_EngineObjetcs* eng, STNixContextItf* ctx, const NixUI16 pregeneratedSources){
    NixBOOL r = NIX_FALSE;
    eng->ctx                        = *ctx;
    ...
    eng->sourcesArr = (STNix_source*)(*ctx->mem.malloc)(sizeof(STNix_source) * eng->sourcesArrSize, "sourcesArr");
    eng->sourcesArr[0].regInUse        = 0;
    //
    eng->buffersArrUse                = 1; //Buffer index zero is reserved
    eng->buffersArrSize                = 1 + NIX_BUFFERS_GROWTH;
    eng->buffersArr = (STNix_bufferAL*)(*ctx->mem.malloc)(sizeof(STNix_bufferAL) * eng->buffersArrSize, "buffersArr");
...
NixBOOL nixInit(STNixContextItf* ctx, STNix_Engine* engAbs, const NixUI16 pregeneratedSources){
    NixBOOL r = NIX_FALSE;
    STNix_EngineObjetcs* eng = (STNix_EngineObjetcs*)(*ctx->mem.malloc)(sizeof(STNix_EngineObjetcs), "STNix_EngineObjetcs");
    memset(eng, 0, sizeof(*eng));
    engAbs->o = eng;
    if(!nixInitBase_(eng, ctx, pregeneratedSources)){
...
#       ifdef NIX_DEFAULT_API_AVFAUDIO
        ...
        } else if(eng->api.engine.alloc == NULL){
            NIX_PRINTF_ERROR("nixAVAudioEngine_getApiItf provided NULL for eng->api.engine.create.\n");
        } else {
            eng->apiEngine = (*eng->api.engine.alloc)(ctx);
            if(eng->apiEngine.ptr == NULL){
...
NixBOOL nixInitWithOpenAL(STNix_Engine* engAbs, const NixUI16 pregeneratedSources){
    ...
    if(!nixInitBase_(eng, pregeneratedSources)){
        ...
        if(!nixOpenALEngine_getApiItf(&eng->api)){
            ...
            eng->apiEngine = (*eng->api.engine.alloc)();
            if(eng->apiEngine.ptr == NULL){
                NIX_PRINTF_ERROR("nixOpenALEngine_getApiItf::eng->api.engine.create provided NULL engine.\n");
            } else {
                NIX_PRINTF_INFO("nixOpenALEngine_getApiItf::eng->api.engine.create OK.\n");
...
void nixPrintCaps(STNix_Engine* engAbs){
    STNix_EngineObjetcs* eng = (STNix_EngineObjetcs*)engAbs->o;
    if(eng->apiEngine.ptr != NULL && eng->api.engine.printCaps != NULL){
...
void nixFinalize(STNix_Engine* engAbs){
	...
	if(eng->sourcesArr != NULL){
        ...
        (*eng->ctx.mem.free)(eng->sourcesArr); eng->sourcesArr = NULL;
	}
	//Destroy buffers
	if(eng->buffersArr != NULL){
        ...
        (*eng->ctx.mem.free)(eng->buffersArr); eng->buffersArr = NULL;
	}
    ...
    if(eng->apiEngine.ptr != NULL){
        if(eng->api.engine.free != NULL){
            (*eng->api.engine.free)(eng->apiEngine);
            eng->apiEngine = (STNixApiEngineRef)STNixApiEngineRef_Zero;
        }
    }
	//
    (*eng->ctx.mem.free)(eng);
	engAbs->o = NULL;
}
...
NixBOOL nixContextIsActive(STNix_Engine* engAbs){
	NixBOOL r = NIX_FALSE;
    STNix_EngineObjetcs* eng = (STNix_EngineObjetcs*)engAbs->o;
    if(eng->apiEngine.ptr != NULL && eng->api.engine.ctxIsActive != NULL){
...
NixBOOL nixctxActivate(STNix_Engine* engAbs){
	NixBOOL r = NIX_FALSE;
    STNix_EngineObjetcs* eng = (STNix_EngineObjetcs*)engAbs->o;
    if(eng->apiEngine.ptr != NULL && eng->api.engine.ctxActivate != NULL){
...
NixBOOL nixctxDeactivate(STNix_Engine* engAbs){
	NixBOOL r = NIX_FALSE;
    STNix_EngineObjetcs* eng = (STNix_EngineObjetcs*)engAbs->o;
    if(eng->apiEngine.ptr != NULL && eng->api.engine.ctxDeactivate != NULL){
...
NixUI16 __nixSourceAssign(STNix_Engine* engAbs, NixUI8 lookIntoReusable, NixUI8 audioGroupIndex, PTRNIX_SourceReleaseCallback releaseCallBack, void* releaseCallBackUserData, const NixUI16 queueSize, PTRNIX_StreamBufferUnqueuedCallback bufferUnqueueCallback, void* bufferUnqueueCallbackData){
    ...
    if(eng->api.source.alloc != NULL){
        STNix_AudioGroup* grp = &__nixSrcGroups[audioGroupIndex];
        STNixApiSourceRef src = (*eng->api.source.alloc)(eng->apiEngine);
        if(src.ptr == NULL){
            ...
            if(newSourceIndex == 0){
                if(eng->api.source.free != NULL){
                    (*eng->api.source.free)(src);
                    src = (STNixApiSourceRef)STNixApiSourceRef_Zero;
                }
            } else {
	...
	NIX_GET_SOURCE_START(eng, sourceIndex, source) {
		...
		if(bufferIndex == 0){
			...
			NIX_GET_BUFFER_START(eng, bufferIndex, buffer) {
                {
                    if(source->apiSource.ptr == NULL){
                        NIX_PRINTF_ERROR("nixSourceSetBuffer::source->apiSource.ptr is NULL.\n");
	...
	NIX_GET_SOURCE_START(eng, sourceIndex, source) {
		NIX_GET_BUFFER_START(eng, streamBufferIndex, buffer) {
			...
			if(buffer->bufferDesc.state == ENNixBufferState_LoadedForPlay){
                if(source->apiSource.ptr == NULL){
                    NIX_PRINTF_ERROR("nixSourceStreamAppendBuffer::source->apiSource.ptr is NULL.\n"); NIX_ASSERT(0)
                } else if(buffer->apiBuff.ptr == NULL){
                    NIX_PRINTF_ERROR("nixSourceStreamAppendBuffer::buffer->apiBuff.ptr is NULL.\n"); NIX_ASSERT(0)
	...
	if(groupIndex < NIX_AUDIO_GROUPS_SIZE){
		...
		for(i=0; i < use; i++){
            ...
            if(source->regInUse && source->audioGroupIndex == groupIndex){
                if(source->apiSource.ptr != NULL && source->api.setVolume != NULL){
	...
	if(buffIndex != 0){
		...
		if(buffIndex < eng->buffersArrUse){
			...
			if(eng->buffersArr[buffIndex].regInUse){
				...
				STNixAudioDesc* audioDesc = &bufferDesc->audioDesc;
...
void nixCaptureBufferFilledCallback_(STNixApiEngineRef apiEng, STNixApiRecorderRef rec, const STNixAudioDesc audioDesc, const NixUI8* audioData, const NixUI32 audioDataBytes, const NixUI32 audioDataSamples, void* userdata){
...
NixBOOL nixCaptureInit(STNix_Engine* engAbs, const STNixAudioDesc* audioDesc, const NixUI16 buffersCount, const NixUI16 samplesPerBuffer, PTRNIX_CaptureBufferFilledCallback bufferCaptureCallback, void* bufferCaptureCallbackUserData){
    ...
    if(eng->apiEngine.ptr != NULL && eng->apiRecorder.ptr == NULL && eng->api.recorder.alloc != NULL && buffersCount != 0 && samplesPerBuffer != 0){
        STNixApiRecorderRef recorder = (*eng->api.recorder.alloc)(eng->apiEngine, audioDesc, buffersCount, samplesPerBuffer);
        if(recorder.ptr != NULL){
            //set callback
            if(eng->api.recorder.setCallback != NULL){
                if(!(*eng->api.recorder.setCallback)(recorder, nixCaptureBufferFilledCallback_, eng)){
                    ...
                    eng->apiRecorder = recorder; recorder = (STNixApiRecorderRef)STNixApiRecorderRef_Zero; //consume
        ...
        //release (if not consumed)
        if(recorder.ptr != NULL){
            if(eng->api.recorder.free != NULL){
                (*eng->api.recorder.free)(recorder);
            }
            recorder = (STNixApiRecorderRef)STNixApiRecorderRef_Zero;
        }
    }
	return r;
}
...
void nixCaptureFinalize(STNix_Engine* engAbs){
    ...
    if(eng->apiRecorder.ptr != NULL){
        if(eng->api.recorder.stop != NULL){
            (*eng->api.recorder.stop)(eng->apiRecorder);
        }
        if(eng->api.recorder.free != NULL){
            (*eng->api.recorder.free)(eng->apiRecorder);
        }
        eng->apiRecorder = (STNixApiRecorderRef)STNixApiRecorderRef_Zero;
    }
	//Destroy capture buffers
	if(eng->buffersCaptureArr != NULL){
		NixUI16 i; const NixUI16 useCount = eng->buffersCaptureArrSize;
		for(i=0; i < useCount; i++){
            (*eng->ctx.mem.free)(eng->buffersCaptureArr[i].dataPointer);
		}
        (*eng->ctx.mem.free)(eng->buffersCaptureArr);
		eng->buffersCaptureArr				= NULL;
		eng->buffersCaptureArrSize			= 0;
		eng->buffersCaptureArrFilledCount	= 0;
	}
...
void* nixFmtConverter_alloc(STNixContextItf* ctx){
    STNix_FmtConverter* r = (STNix_FmtConverter*)(*ctx->mem.malloc)(sizeof(STNix_FmtConverter), "STNix_FmtConverter");
